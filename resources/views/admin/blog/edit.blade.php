<x-admin-layout>
    <div class="px-6 py-4">
        <h3 class="text-gray-700 text-3xl font-medium">Edit Blog Post</h3>
    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="md:col-span-2 space-y-6">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-400">Slug: {{ $post->slug }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-gray-700 uppercase tracking-wider mb-2">Content</label>
                            <textarea name="content" rows="15" required
                                      class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all">{{ old('content', $post->content) }}</textarea>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="p-6 bg-gray-50 rounded-2xl border border-gray-100">
                            ...
                            @if($post->featured_image)
                                <img src="{{ $post->featured_image_url }}" alt="" class="w-full h-32 object-cover rounded-xl mb-4 shadow-sm">
                            @endif
                            ...
                            <p class="mt-2 text-xs text-gray-400">Max size 2MB. Leave blank to keep current.</p>
