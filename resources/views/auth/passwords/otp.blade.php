        ...
        <div class="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:flex-none lg:px-20 xl:px-24">
            <div class="mx-auto w-full max-w-md lg:w-96">
                <div class="text-center mb-8">
                    <a href="{{ route('password.request') }}" class="inline-flex items-center transition-colors" style="color: #001F3F;" onmouseover="this.style.color='#C6A664'" onmouseout="this.style.color='#001F3F'">
                    ...
                    <form action="{{ route('password.otp.verify.post') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input id="code" name="code" type="text" maxlength="6" pattern="[0-9]{6}" required 
                                   class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#001F3F] focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm tracking-widest text-center text-2xl font-bold"
                    ...
                    <div class="mt-6 text-center">
                        <p class="text-sm text-gray-600">
                            ...
                            <form action="{{ route('password.email') }}" method="POST" class="inline">
                                @csrf
                                <input type="hidden" name="email" value="{{ $email }}">
                                <button type="submit" class="text-[#001F3F] font-semibold hover:underline">Resend OTP</button>
                            </form>
                        </p>
                    </div>
                </div>
