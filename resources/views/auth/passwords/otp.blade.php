        ...
        <div class="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:flex-none lg:px-20 xl:px-24">
            <div class="mx-auto w-full max-w-md lg:w-96">
                <div class="text-center mb-8">
                    <a href="{{ route('password.request') }}" class="inline-flex items-center transition-colors" style="color: #A85C2E;" onmouseover="this.style.color='#2B1810'" onmouseout="this.style.color='#A85C2E'">
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-8">
                    <div class="text-center">
                        ...
                        @if (session('status'))
                            <div class="mb-4 font-medium text-sm text-green-600 p-4 bg-green-50 rounded-lg text-center">
                                {{ session('status') }}
                            </div>
                        @endif
                    ...
                    <form action="{{ route('password.otp.verify.post') }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <input id="code" name="code" type="text" maxlength="6" pattern="[0-9]{6}" required 
                                   class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#A85C2E] focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm tracking-widest text-center text-2xl font-bold"
                    ...
                    <div class="mt-6 text-center">
                        <p class="text-sm text-gray-600">
                            ...
                            <form action="{{ route('password.email') }}" method="POST" class="inline">
                                @csrf
                                <input type="hidden" name="email" value="{{ $email }}">
                                <button type="submit" class="text-[#A85C2E] font-semibold hover:underline">Resend OTP</button>
                            </form>
                        </p>
                    </div>
                </div>
