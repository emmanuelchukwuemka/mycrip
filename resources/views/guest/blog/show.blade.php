<x-app-layout>
    <div class="bg-white min-h-screen">
        <!-- Post Header -->
        <div class="relative py-24 bg-[#A85C2E] overflow-hidden">
            <div class="absolute inset-0 opacity-20">
                <img src="{{ $post->featured_image_url }}" alt="" class="w-full h-full object-cover filter blur-sm">
            </div>
            <div class="relative max-w-4xl mx-auto px-4 text-center">
                <a href="{{ route('blog.index') }}" class="inline-flex items-center text-[#2B1810] font-bold text-sm uppercase tracking-widest mb-8 hover:underline">
                ...
                <div class="flex items-center justify-center gap-6 text-gray-300 font-bold uppercase tracking-widest text-xs">
                    <div class="flex items-center">
                        <svg class="w-4 h-4 mr-2 text-[#2B1810]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        {{ $post->published_at->format('M d, Y') }}
                    </div>
                    <div class="flex items-center">
                        <svg class="w-4 h-4 mr-2 text-[#2B1810]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        By {{ $post->author->name }}
                    </div>
                </div>
            </div>
        ...
        <div class="max-w-4xl mx-auto px-4 -mt-16 relative z-10">
                    ...
                    <div class="mt-16 pt-8 border-t border-gray-100 flex flex-wrap items-center justify-between gap-6">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex gap-2">
                                <a href="#" class="p-3 bg-gray-50 text-gray-400 hover:bg-[#A85C2E] hover:text-white rounded-2xl transition-all duration-300">
            ...
            <div class="bg-gray-50 rounded-3xl p-8 mb-20 border border-gray-100 flex flex-col md:flex-row items-center md:items-start gap-8">
                ...
                <div class="text-center md:text-left">
                    <h5 class="text-xl font-bold text-[#A85C2E] mb-2">{{ $post->author->name }}</h5>
                    <p class="text-gray-600 leading-relaxed italic">
                        {{ $post->author->bio ?? 'Passionate real estate expert and contributing writer for the Villa Africa platform.' }}
                    </p>
                </div>
            </div>
            ...
            @if($relatedPosts->count() > 0)
                <div class="mb-20">
                    <h3 class="text-3xl font-bold text-[#A85C2E] mb-12">Related Articles</h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        @foreach($relatedPosts as $related)
                            <article class="flex flex-col">
                                ...
                                <h4 class="text-lg font-bold text-[#A85C2E] mb-2 line-clamp-2 hover:text-[#2B1810] transition-colors">
                                    <a href="{{ route('blog.show', $related->slug) }}">{{ $related->title }}</a>
                                </h4>
                                <p class="text-xs font-bold text-[#2B1810] uppercase tracking-widest">{{ $related->published_at->format('M d, Y') }}</p>
                            </article>
                        @endforeach
                    </div>
                </div>
