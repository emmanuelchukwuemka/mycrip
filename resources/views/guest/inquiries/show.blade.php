    ...
    <div class="bg-gray-50 min-h-screen py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex mb-8 items-center text-sm font-bold text-gray-400 uppercase tracking-widest" aria-label="Breadcrumb">
                <a href="{{ route('inquiries.index') }}" class="hover:text-[#A85C2E] transition-colors">My Inquiries</a>
                <svg class="w-4 h-4 mx-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                </svg>
                <span class="text-[#2B1810]">Inquiry Details</span>
            </nav>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden">
                ...
                <div class="relative h-64">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-[#A85C2E] to-transparent"></div>
                    <div class="absolute bottom-8 left-8 right-8">
                        <div class="flex items-center space-x-2 mb-2">
                            <span class="px-3 py-1 bg-[#2B1810] text-white text-[10px] font-black uppercase tracking-widest rounded-lg">
                    ...
                    <a href="{{ route('properties.show', $inquiry->property->id) }}" 
                       class="absolute top-8 right-8 p-4 bg-white/10 backdrop-blur-md rounded-2xl text-white hover:bg-[#2B1810] transition-all transform hover:rotate-12">
                            ...
                            <div>
                                ...
                                <div class="space-y-6">
                                    <div class="flex items-start space-x-4">
                                        <div class="w-10 h-10 rounded-2xl bg-[#A85C2E] flex items-center justify-center text-white text-xs font-bold shrink-0">
                                            ME
                                        </div>
                                        <div class="flex-grow pt-1">
                                            <div class="flex items-center justify-between mb-1">
                                                <p class="text-sm font-bold text-[#A85C2E]">Inquiry Submitted</p>
                                    ...
                                    @if($inquiry->status === 'responded')
                                        <div class="flex items-start space-x-4">
                                            <div class="w-10 h-10 rounded-2xl bg-[#2B1810] flex items-center justify-center text-white text-xs font-bold shrink-0">
                                                {{ strtoupper(substr($inquiry->property->user->name, 0, 1)) }}
                                            </div>
                                            <div class="flex-grow pt-1">
                                                <div class="flex items-center justify-between mb-1">
                                                    <p class="text-sm font-bold text-[#A85C2E]">Agent Responded</p>
                        ...
                        <div class="space-y-8">
                            <div>
                                ...
                                <div class="bg-white rounded-3xl p-6 border-2 border-gray-100 flex flex-col items-center text-center shadow-sm">
                                    <div class="w-20 h-20 rounded-full bg-gradient-to-tr from-[#A85C2E] to-[#003366] p-1 mb-4 shadow-lg">
                                        <div class="w-full h-full rounded-full bg-white flex items-center justify-center">
                                            <div class="w-[90%] h-[90%] rounded-full bg-[#A85C2E] flex items-center justify-center text-white text-2xl font-black">
                                                {{ strtoupper(substr($inquiry->property->user->name, 0, 1)) }}
                                            </div>
                                        </div>
                                    </div>
                                    <h3 class="text-lg font-black text-[#A85C2E]">{{ $inquiry->property->user->name }}</h3>
                                    <p class="text-xs font-bold text-[#2B1810] uppercase tracking-widest mt-1">Verified Agent</p>
                                    
                                    <div class="w-full h-px bg-gray-100 my-6"></div>
                                    
                                    <a href="{{ route('chat.index') }}" 
                                       class="w-full py-4 bg-[#A85C2E] text-white font-black rounded-2xl hover:bg-[#2B1810] transition-all shadow-xl hover:shadow-[#A85C2E]/20 active:scale-95 flex items-center justify-center">
                            ...
                            <div class="bg-gradient-to-br from-[#A85C2E] to-[#2B1810] rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
                                <div class="absolute -right-4 -top-4 w-24 h-24 bg-white/5 rounded-full blur-2xl"></div>
                                <h3 class="text-lg font-black mb-2 relative z-10 text-[#2B1810]">Need Help?</h3>
