            ...
            <form action="{{ route('agent.properties.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12" x-data="{ 
                        ...
                        <div class="pt-6 space-y-6">
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="url" name="virtual_tour_url" id="virtual_tour_url" value="{{ old('virtual_tour_url') }}"
                                        class="w-full px-4 py-4 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-[#2B1810]/20 focus:border-[#2B1810] transition-all duration-300 placeholder-gray-400 outline-none shadow-sm"
                    ...
                    <div>
                        ...
                        <div class="group relative border-4 border-dashed border-gray-100 rounded-3xl p-8 text-center hover:border-[#2B1810]/30 hover:bg-gray-50/50 transition-all duration-300">
                            ...
                            <label for="images" class="cursor-pointer">
                                <div class="text-center">
                                    <div class="mx-auto w-20 h-20 bg-gray-50 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
                                        <svg class="h-10 w-10 text-gray-300 group-hover:text-[#2B1810]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                        </svg>
                                    </div>
                                    <div class="text-sm text-gray-600">
                                        <span class="font-bold text-[#A85C2E] hover:text-[#2B1810]">Click to upload</span> or drag and drop
                                    </div>
                                    <p class="text-xs text-gray-400 mt-2 font-medium">PNG, JPG, JPEG, WEBP</p>
                                    <p class="text-xs text-[#2B1810] mt-2 font-bold bg-[#2B1810]/10 inline-block px-3 py-1 rounded-full" id="file-count">No files selected</p>
                                </div>
                            </label>
                        </div>
                ...
                <div class="border-t border-gray-100 pt-10 mb-10" x-data="{ 
                    subGalleries: [],
                    addGallery() {
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-xl font-bold text-[#A85C2E] flex items-center">
                            ...
                            Sub-Galleries (Optional)
                        </h3>
                        <button type="button" @click="addGallery()" 
                                class="inline-flex items-center px-4 py-2 bg-[#2B1810] text-white text-sm font-bold rounded-xl hover:bg-[#A85C2E] transition-all shadow-lg shadow-[#2B1810]/20">
                    ...
                    <p class="text-gray-500 text-sm mb-6">Create separate galleries for specific areas like "Master Bedroom", "Kitchen", or "Hotel Lobby".</p>

                    <div class="space-y-6">
                        <template x-for="(gallery, index) in subGalleries" :key="gallery.id">
                            <div class="p-6 bg-gray-50 rounded-3xl border border-gray-100 relative group transition-all hover:shadow-md">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div class="md:col-span-1">
                                        <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Gallery Label</label>
                                        <input type="text" :name="'sub_galleries[' + index + '][label]'" 
                                               x-model="gallery.label"
                                               placeholder="e.g. Master Bedroom"
                                               class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-4 focus:ring-[#2B1810]/10 focus:border-[#2B1810] outline-none transition-all text-sm font-semibold" required>
                                    </div>
                                    <div class="md:col-span-2">
                                        <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Upload Gallery Images</label>
                                        <input type="file" :name="'sub_galleries[' + index + '][images][]'" multiple accept="image/*"
                                               class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:ring-4 focus:ring-[#2B1810]/10 outline-none transition-all text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-[#A85C2E] file:text-white hover:file:bg-[#2B1810] cursor-pointer" required>
                                    </div>
                                </div>
                            </div>
                        </template>

                        <div x-show="subGalleries.length === 0" class="text-center py-10 border-2 border-dashed border-gray-100 rounded-3xl bg-gray-50/30">
                            <p class="text-gray-400 text-sm font-medium">No sub-galleries added yet. Click "Add New Gallery" to categorize your photos.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="border-t border-gray-100 pt-10 mb-12">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 mb-8">
                        ...
                        <div>
                            ...
                            <input type="number" name="shops" id="shops" value="{{ old('shops') }}" min="0" max="100"
                                class="w-full px-4 py-4 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-[#2B1810]/20 focus:border-[#2B1810] transition-all duration-200 outline-none"
                                placeholder="0">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Warehouses</label>
                            <input type="number" name="warehouses" id="warehouses" value="{{ old('warehouses') }}" min="0" max="100"
                                class="w-full px-4 py-4 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-[#2B1810]/20 focus:border-[#2B1810] transition-all duration-200 outline-none"
                                placeholder="0">
                        </div>
                    </div>
                    ...
                    <div x-show="category === 'hotel' || category === 'lodge'" x-transition class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 p-6 bg-[#2B1810]/5 rounded-3xl border border-[#2B1810]/20">
                        ...
                        <div>
                            ...
                            <select name="security_level" id="security_level" 
                                ...
                                <option value="enhanced" {{ old('security_level') === 'enhanced' ? 'selected' : '' }}>Enhanced (CCTV)</option>
                                <option value="premium" {{ old('security_level') === 'premium' ? 'selected' : '' }}>Premium (24/7 Guards + CCTV)</option>
                                <option value="elite" {{ old('security_level') === 'elite' ? 'selected' : '' }}>Elite (Armed Security)</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Parking Capacity (Cars)</label>
                            <input type="number" name="parking_capacity" id="parking_capacity" value="{{ old('parking_capacity') }}"
                                class="w-full px-4 py-4 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-[#2B1810]/20 focus:border-[#2B1810] transition-all duration-200 outline-none"
                                placeholder="e.g. 20">
                        </div>
                    </div>
