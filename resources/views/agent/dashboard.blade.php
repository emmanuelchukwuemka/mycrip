...
<div class="space-y-6">
    ...
    <div class="relative overflow-hidden rounded-2xl px-8 py-6 flex items-center justify-between gap-6"
         style="background:linear-gradient(135deg,#A85C2E 0%,#2B1810 60%,#001a35 100%);">
        ...
        <div class="relative z-10 flex items-center gap-5">
            ...
            <div class="relative flex-shrink-0">
                ...
                {{-- Online dot --}}
                <span class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-emerald-400 ring-2 ring-[#A85C2E]"></span>
            </div>
        ...
        <div class="relative z-10 flex items-center gap-3">
            <a href="{{ route('agent.properties.create') }}"
               ...
               style="background:linear-gradient(135deg,#2B1810,#a8894e); color:#A85C2E;">
        ...
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 group">
            <div class="flex items-start justify-between">
                ...
                <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:scale-110"
                     style="background:rgba(43, 24, 16,0.1)">
                    <svg class="w-5 h-5 text-[#2B1810]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75">
            ...
            <div class="mt-4 pt-3 border-t border-gray-50 flex items-center justify-between">
                ...
                <a href="{{ route('requests.index') }}" class="text-[10px] font-bold text-[#2B1810] bg-[#2B1810]/10 px-2 py-0.5 rounded uppercase tracking-tighter hover:bg-[#2B1810] hover:text-white transition-all">Browse Wall</a>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 group relative overflow-hidden">
            ...
            @if($newInquiries > 0)
                <div class="absolute inset-0 opacity-[0.04] pointer-events-none"
                     style="background:radial-gradient(circle at top right,#2B1810,transparent)"></div>
            @endif
            <div class="relative flex items-start justify-between">
                <div>
                    <p class="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">New Inquiries</p>
                    <p class="text-3xl font-black leading-none" style="color:#2B1810">{{ $newInquiries }}</p>
                </div>
                <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:scale-110"
                     style="background:rgba(43, 24, 16,0.12)">
                    <svg class="w-5 h-5" style="color:#2B1810" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75"/>
                    </svg>
                </div>
            </div>
            <div class="mt-4 pt-3 border-t border-gray-50">
                <a href="{{ route('agent.inquiries.index') }}" class="text-xs font-semibold hover:underline transition-colors" style="color:#2B1810">View all →</a>
            </div>
        </div>
    </div>
    ...
    @if(!$currentSubscription || !$currentSubscription->isActive())
        <div class="flex items-center justify-between gap-4 px-6 py-4 rounded-2xl border"
             style="background:rgba(43, 24, 16,0.06); border-color:rgba(43, 24, 16,0.25)">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                     style="background:rgba(43, 24, 16,0.15)">
                    <svg class="w-4 h-4" style="color:#2B1810" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mt-6">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-50 bg-[#FDFCF9]/50">
                    <h2 class="text-sm font-bold text-gray-800 tracking-wide flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full bg-[#2B1810]"></span>
                        Recent Buyer Requests
                    </h2>
                    <a href="{{ route('requests.index') }}"
                       class="text-xs font-semibold hover:underline px-3 py-1 bg-[#2B1810]/10 rounded-full text-[#2B1810]" style="color:#2B1810">Buyer Wall →</a>
                </div>
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex-1">
                                <div class="flex items-center gap-3 mb-1">
                                    <span class="text-[10px] font-black uppercase tracking-widest text-[#2B1810] bg-[#2B1810]/5 px-2 py-0.5 rounded border border-[#2B1810]/10">{{ $req->category }}</span>
                            ...
                            <div class="text-right shrink-0">
                                ...
                                <a href="{{ route('chat.start', ['property_id' => null, 'agent_id' => auth()->id(), 'user_id' => $req->user_id]) }}" 
                                   class="text-[10px] font-black text-[#A85C2E] uppercase hover:text-[#2B1810] transition-colors flex items-center gap-1">
                                    Contact
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-gray-50">
                    ...
                    <a href="{{ route('agent.inquiries.index') }}" class="text-xs font-semibold hover:underline" style="color:#2B1810">View All →</a>
                </div>
                <div class="divide-y divide-gray-50">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center justify-between gap-2">
                                        ...
                                        @if($inquiry->status === 'new')
                                            <span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background:#2B1810"></span>
                                        @endif
                                    </div>
                    ...
                    @foreach([
                        ...
                        <a href="{{ route($link['route']) }}"
                           class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-colors group">
                            <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors group-hover:bg-[#A85C2E]"
                                 style="background:rgba(168, 92, 46,0.07)">
                                <svg class="w-3.5 h-3.5 group-hover:text-white transition-colors" style="color:#A85C2E"
