...
<div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 group h-full flex flex-col">
    ...
    <div class="relative overflow-hidden flex-shrink-0">
        ...
        <div class="absolute top-4 right-4 flex space-x-2">
            ...
            <!-- Share Button -->
            <button onclick="navigator.clipboard.writeText('{{ route('properties.show', $property->id) }}')" class="bg-white/90 text-gray-600 p-2 rounded-full hover:bg-white hover:text-indigo-600 transition-all duration-300 shadow-lg transform hover:scale-110" title="Share property">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-5 flex flex-col flex-grow">
        <div class="flex justify-between items-start mb-3">
            <div class="flex-grow">
                <h3 class="text-lg font-bold text-gray-900 group-hover:text-indigo-600 transition-colors duration-300 line-clamp-1">
                    {{ $property->title }}
                </h3>
                <p class="text-gray-600 text-sm flex items-center mt-1">
                    <svg class="h-4 w-4 mr-1 text-indigo-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="text-right ml-2 flex-shrink-0">
                <p class="text-xl font-bold text-indigo-600">{{ $property->formatted_price }}</p>
            </div>
        </div>
        ...
        <div class="border-t pt-3 mt-auto flex justify-between items-center">
            <div class="flex items-center">
                ...
                @else
                    <div class="h-8 w-8 rounded-full bg-indigo-100 flex items-center justify-center border-2 border-indigo-200 flex-shrink-0">
            ...
            <a href="{{ route('properties.show', $property->id) }}" class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:from-indigo-700 hover:to-purple-700 transform hover:scale-105 transition-all duration-300 shadow-md">
                View Details
            </a>
        </div>
    </div>
